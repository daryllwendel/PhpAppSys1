            ...
            @foreach ($mydesign as $item)
                <div class="product-card">
                    ...
                    <div class="product-info">
                        ...
                        @if ($item->viewStatus == 'pending')
                            <p>This product is pending for approval</p>
                        @elseif ($item->viewStatus == 'approved')
                            <button class="buy-button" data-id1="{{ $item->productId }}"
                                data-name1="{{ $item->name }}" data-price1="{{ $item->price }}"
                                data-type1="{{ $item->type }}" data-printtype1="{{ $item->printType }}"
                                data-img1="{{ asset('storage/' . $item->productImg) }}"
                                data-status1="{{ $item->status }}">Buy</button>
                        @endif
